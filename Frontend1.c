...
void allegro_read_input(allegro_t* p, game_t *g, int* move, int* pause, int* reset, int* quit){
    ...
    while (al_get_next_event(p->queue, &ev)){ 
        ...
        //tecla fue soltada 
            ...
            if ((k == ALLEGRO_KEY_LEFT) && (*move == -1)) {
            *move = 0;
            }
            if ((k == ALLEGRO_KEY_RIGHT) && (*move == 1)) {
                *move = 0;
            }
        }
        //ventana cerrada
        ...
        //mouse
            ...
            float x_celda = ev.mouse.x / p->sx; //convierto a pixel 
...
void allegro_draw(allegro_t* p, game_t* g){
    ...
    allegro_play_sfx(p, g); 

    ALLEGRO_BITMAP* back = al_get_backbuffer(p->display); //backbuffer
    al_set_target_bitmap(back);
    al_draw_bitmap(p->bmp_background, 0, 0, 0);
    al_clear_to_color(al_map_rgb(12, 20, 80)); //pongo el fondo color azul
