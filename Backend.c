...
void move_bola(game_t* g, float time){
    ...
    #else
        //raspi, adapto movimiento 
        float max_step = 4.0;   
        float total = b->speed * time;
        int steps = ceilf(total / max_step);
        if (steps < 1){
            steps = 1;
        }

        for (int s = 0; s < steps; s++) {
            float sub_dt = time / steps;

            b->xprev = b->p.x;
            b->yprev = b->p.y;

            b->p.x += b->vel.x * sub_dt;
            b->p.y += b->vel.y * sub_dt;
        }
    #endif
    ...
    for (int i = 0; i < BR_ROWS; i++) {       
        for (int j = 0; j < BR_COLS; j++) {   
            ...
            if (niv == 1) {
                ...
                #ifdef PI_16x16
                    //especial para la raspi
                    int t = 2; //grosor techo
                    if (i < t || j < t || j >= BR_COLS - t) {
                        present = 1; //bordes y techo
                    } 
                    else {
                        present = ((i + j) % 2 == 0); //relleno Ajedrez
                    }
                #else
                    int t = 2; //grosor del arco 
                    if (i < t || j < t || j >= BR_COLS - t) {
                        present = 1; //borde grueso
                    } 
                    else {
                        present = ((i + j) % 2 == 0); //relleno ajedrez
                    }
...
int ball_hit(game_t* g, float rx, float ry, float rw, float rh, int is_vaus){
    ...
    int overlap_now = (b->p.x + b->radio > rx) && (b->p.x - b->radio < rx + rw) && (b->p.y + b->radio > ry) && (b->p.y - b->radio < ry + rh);
        ...
        if (!overlap_now) {
            ...
            int cruzo_vertical = ((y0 + b->radio <= top) && (y1 - b->radio >= top)) || ((y0 - b->radio >= bottom) && (y1 + b->radio <= bottom));

            int x_en_rango_aprox = (fminf(x0, x1) + b->radio <= right) && (fmaxf(x0, x1) - b->radio >= left);

            if (cruzo_vertical && x_en_rango_aprox) {
                //rebotar como choque ver
                if (y0 < top){
                    b->p.y = top - b->radio;
                }
                else if (y0 > bottom){
                    b->p.y = bottom + b->radio;
                }
...
void game_step(game_t* p, float time, int move, int pause, int reset){
    ...
    stage_update(p, time);
    //si no estamos jugando, saltá la física de la bola
    if (p->stage != STAGE_PLAY || p->paused){
